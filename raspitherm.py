...
def displayMode():
    ...
    mode_buttons = {'PWR ->':(280,40), 'CAM ->':(280,100), 'FLIR->':(280,160), 'OVERLAY->':(260,220)}
...
def overlay():
	...
	while loop:
		...
		draw.rectangle((0,0,F_WIDTH,F_HEIGHT), fill=color)
		
		if not overlay == None:
			CAMERA.remove_overlay(overlay)
		overlay = CAMERA.add_overlay(pad.tobytes(), size=pad.size, layer=3, alpha=128)
...
while True:
	...
	if GPIO.input(BTN2) == GPIO.LOW:
                # CAMERA
		CAMERA_DISPLAYING = camera(CAMERA_DISPLAYING)
		#displayMode()
		time.sleep(0.5)
